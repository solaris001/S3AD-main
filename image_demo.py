...
def parse_args():
    ...
    parser.add_argument("--save_to", default="results_tiles_best", help="Save the results json to")
    parser.add_argument(
...
def detector_infer_imgs(imgs,model):
    results= []
    for img in imgs:
        #print(img)
        # test a single image
...
def main(args):
    ...
    batches = [imgs[n:n+10] for n in range(0, len(imgs), 10)]

    results= []
    for batch in batches:
        result = inference_detector(model, batch)
        results += result
        print(len(result),np.asarray(results)[:,0].shape)
    '''
    for img in imgs:
        print(img)
        # test a single image
    ...
    with io.open('./'+args.save_to+'.json', 'w', encoding='utf-8') as f:
        str_ = json.dumps(results, indent=4, sort_keys=True, separators=(',', ': '), ensure_ascii=False,cls=NumpyEncoder)
